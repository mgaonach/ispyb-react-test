 ...
 */
export function validateContainers(containers: ShippingContainer[], proposalSamples: ProposalSample[]) {
  const errors: string[] = [];
  ...
  const sampleKeys = containers.flatMap((c) => c.sampleVOs).map((sample) => getSampleKey(sample.crystalVO?.proteinVO.acronym, sample.name));
  ...
  const proposalSampleKeys = proposalSamples.filter((s) => !containerIds.includes(s.Container_containerId)).map((s) => getSampleKey(s.Protein_acronym, s.BLSample_name));
  ...
  if (proposalDuplicates.length == 1) {
    ...
    errors.push(`non-unique sample keys in proposal: ${proposalDuplicates.join(', ')} `);
  }
  return errors;
}
